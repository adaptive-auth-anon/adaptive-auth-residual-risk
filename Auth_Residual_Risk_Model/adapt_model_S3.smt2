...
(assert
  (ite ControlledPrescriptions_Delete
    (and (>= ControlledPrescriptionsSensitivity 0.87) (<= ControlledPrescriptionsSensitivity 0.9))
    (ite ControlledPrescriptions_Update
      (and (>= ControlledPrescriptionsSensitivity 0.84) (<= ControlledPrescriptionsSensitivity 0.87))
...
(assert (= AssetValue
  (+ (* Diagnosis DiagnosisSensitivity 0.2)
     ...
     (* PatientRecords PatientRecordsSensitivity 0.2))
))
...
(assert (ite (> Location 0)  
     (and 
        (>= ConfPriority 0.8)         ;; Location can increase confidentiality priority
        ...
        (= EffectPriority 0.5)        ;; Slight usability concern 
        (>= EfficPriority 0.5)         ;; 
        (= PerformancePriority 0.5))  ;; Slight impact 
      true )) 
...
(assert (ite (> UnknownDevice 0)
  (and
    (>= ConfPriority 0.85)            ;; Personal or unmanaged devices are prone to data leakage — confidentiality is critical
    (>= AuthentPriority 0.8)          ;; Device unfamiliarity raises impersonation risk
    (>= IntegPriority 0.75)           ;; Integrity of submitted/modified data may be questioned
    (>= EffectPriority 0.5)           ;; Potential usability impact 
    (= EfficPriority 0.55)            ;; Effort increased for verification on new device
...
(assert (ite (> UnsecuredWiFi 0)
     (and 
          ...
          (>= AuthentPriority 0.75)    ;; Session hijacking or spoofing possible — higher authenticity needed
...
(assert (ite (> UnsecuredWiFi 0)
  (and
    ...
    (= PSessionAttack 0.7))     ;; Session hijacking very likely over open networks
  true))
...
(assert (ite (> UnknownDevice 0)
  (and
    (= PImpersAttack 0.71)      ;; Impersonation risk high: compromised or spoofed identity
...
(assert (ite (> Location 0)
  (and
    ...
    (>= PSessionAttack 0.65))     ;; Session hijacking risk elevated: access from unknown place
  true))
...
; Floor priorities to avoid 0-division
(assert (and (>= AuthentPriority 0.01)
             (>= ConfPriority 0.01)
             (>= IntegPriority 0.01)
             (>= EfficPriority 0.01)
             (>= EffectPriority 0.01)
             (>= PerformancePriority 0.01)))


;-------- S4 contextual factors ----------

(assert (= Scanner 0))  ;;Laptop lacks scanner
(assert (= Reader 0))   ;; The lack of readder prevents Fingerprint
;(assert (> PoorLighting 0)) ;;Poor lighting prevents Face & Iris
(assert (> Location 0))
...
;; Needs adjustment
(assert (> DiagnoseMedicalConditions 0) )
(assert (> ViewLabResults 0) )
(assert (> Controlled 0) )
(assert (> NonControlled 0))
(assert (> AddEncounterNote 0) )
(assert (> AddReferralNote 0))
(assert (> GenerateReports 0))
