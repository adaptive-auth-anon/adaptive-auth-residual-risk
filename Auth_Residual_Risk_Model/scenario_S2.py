...
def run_with_timeout(seconds, func, *args, **kwargs):
    ...
    Works on Unix/macOS (main thread only).
    Raises _TimeLimit on timeout.
    """
...
if os.path.exists("S2_runs_summary.csv"):
    os.remove("S2_runs_summary.csv")
...
assets = [
    ...
    "PatientDemographics","RadiologyImages","DischargeSummaries",
...
category_operations = {
    "Must": {
        "AccessPatientProfile": ["PatientRecords", "PatientHistory","PatientDemographics"],
        "OrderLabTest": ["PatientRecords", "TestReports"],
        "AdjustCarePlan": ["Diagnosis","PatientRecords"]
    },
    "Nice": {
        "ApproveDischargeSummary": ["PatientRecords", "VisitSummaries","DischargeSummaries"],
        "RequestConsultation": ["ReferralNotes", "EncounterNotes"],
        "CommunicateWithPatient": ["PatientInteractions","PatientCommunications"]
    },
    "Optional": {
        "ViewRadiologyImages": ["PatientRecords", "TestReports","RadiologyImages"],
        "ManageAppointment": ["PatientRecords", "VisitSummaries"],
        "PrintMedicalReport": ["Diagnosis", "PatientRecords", "TreatmentPlan"]
    }
}
...
def print_operations_block(values, title="Operations"):
    print(title + ":")
    print("  "
          f"AccessPatientProfile={fmt(values.get('AccessPatientProfile'))} | "
          f"OrderLabTest={fmt(values.get('OrderLabTest'))} | "
          f"ViewRadiologyImages={fmt(values.get('ViewRadiologyImages'))} | "
          f"ApproveDischargeSummary={fmt(values.get('ApproveDischargeSummary'))} | "
          f"AdjustCarePlan={fmt(values.get('AdjustCarePlan'))} | "
          f"RequestConsultation={fmt(values.get('RequestConsultation'))} | "
          f"ManageAppointment={fmt(values.get('ManageAppointment'))} | "
          f"CommunicateWithPatient={fmt(values.get('CommunicateWithPatient'))} | "
          f"PrintMedicalReport={fmt(values.get('PrintMedicalReport'))}")
...
# ---------------------------------
solver = z3.Solver()
ast = z3.parse_smt2_file("/Users/kabashi/Desktop/The model copy/adapt_model_S2.smt2")
solver.add(ast)
...
def run_residual_with_bound(target_rr, rr_band=0.023, timeout_ms=300000, max_iters=200, min_step=0.012):
    """
    ...
    ast_res = z3.parse_smt2_file("/Users/kabashi/Desktop/The model copy/residual_model_S2.smt2")
    base.add(ast_res)
...
def sweep_residual(step=0.07, start=None):
    """
    ...
    while target >= -1e-9:
        ...
        try:
            # e.g., 25-second cap per sweep attempt — tweak as you like
            ok = run_with_timeout(0.5, run_residual_with_bound, target)
        except _TimeLimit:
            print(f"\n[timeout] Sweep at ResRisk ≤ {target:.2f} exceeded time limit. Skipping the sweep.\n")
            continue  # STOP the whole sweep immediately
...
sweep_residual(step=0.005812313231)  # starts from the adaptive model's ResRisk